...
/* Busy-waits for approximately MS milliseconds. Interrupts need not be turned on.
...
/* Sleeps execution for approximately NS nanoseconds. Interrupts need not be turned on.
...
static void real_time_sleep(int64_t num, int32_t denom) {
  ...
  */
  ...
  if(ticks > 0) {
      ...
      /* We're waiting for at least one full timer tick. Use timer_sleep() because it will yield the CPU to other processes. */                
      timer_sleep(ticks);
